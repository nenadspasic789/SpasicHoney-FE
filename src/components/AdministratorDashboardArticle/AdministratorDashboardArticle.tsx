...
class AdministratorDashboardArticle extends React.Component {
    ...
    private getArticles() {
        api('/api/article/?join=articleFeature&join=articlePrices&join=photos&join=category', 'get', {}, 'administrator')
        .then((res: ApiResponse) => {
            if (res.status === "error" || res.status === "login") {
                this.setLogginState(false);
                return;
            ...
            <Container>
                        ...
                        <Table hover size="sm" bordered>
                            ...
                            <tbody>
                                { this.state.articles.map(article => (
                                    <tr>
                                        <td className="text-center">{ article.articleId }</td>
                                        <td>{ article.name }</td>
                ...
                <Modal size="lg" centered show={ this.state.addModal.visible }
                    ...
                    <Form.Group>
                        ...
                        <Alert variant="danger" className={ this.state.addModal.message ? '' : 'd-none' }>
                ...
                <Modal size="lg" centered show={ this.state.editModal.visible }
                    ...
                    <Modal.Body>
                        ...
                        <Alert variant="danger" className={ this.state.editModal.message ? '' : 'd-none' }>
    ...
    private doAddArticle() {
        const filePicker: any = document.getElementById('add-photo');
        console.log("Ucitao element");
        if (filePicker.files.length === 0) {
            this.setAddModalStringFieldState('message', 'You must select a file to upload!')
            console.log("Nema slike-izlaz");
            console.log(this.state.addModal.message);
            return;
        }
        ...
        .then(async (res: ApiResponse) => {
            ...
            console.log("Ne bi trebalo da se vidi ako nema slike!");
            const articleId: number = res.data.articleId;

            const file = filePicker.files[0]
            const res2 = await this.uploadArticlePhoto(articleId, file);

            if (res2.status !== 'ok') {
                this.setAddModalStringFieldState('message', 'Could not upload this file. It should not be larger than 20mb. Try again!');
                return;
            }
